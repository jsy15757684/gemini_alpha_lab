...
Render 같은 PaaS 는 아웃바운드가 공용 대역이라 등록할 수 없습니다.
...
## 참고: Render 를 계속 쓰고 싶다면

VPS 를 프록시로만 쓰는 방법도 있습니다. VPS 에 프록시(예: tinyproxy)를 올리고
Render 환경변수에 `BITHUMB_PROXY_URL=http://아이디:비번@VPS_IP:3128` 을 넣으면
빗썸 인증 호출만 VPS 를 경유합니다.

다만 **Render 는 컨테이너 디스크가 휘발성**이라 재배포 시 `data/bots.json` 이
사라집니다. 봇이 포지션을 들고 있는 상태에서 재배포하면 추적이 끊깁니다.
24시간 실전 운용에는 권하지 않습니다.
